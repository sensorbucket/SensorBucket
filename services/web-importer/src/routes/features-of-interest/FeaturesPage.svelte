<script lang="ts">
    ...
    import {useReconciliationStore} from "$lib/store/featureOfInterestReconciliation.svelte";
...
<div class="p-4">
    <div class="grid cols-2 gap-4 justify-center w-full">
            ...
            {:else}
                <div class="flex flex-col gap-1">
                    <div class="flex flex-col">
                        ...
                        <small class="block text-xs text-stone-400">Right click a feature of interest to do a single
                            update</small>
                    </div>
        ...
        {#if store.warnings.length > 0}
            <div class="col-span-full bg-amber-100 border border-amber-400 text-amber-700 px-4 py-3 rounded relative"
                 role="alert">
                {#each store.warnings as warning}
                    {warning}
                {/each}
            </div>
        {/if}
...
<Table rows={store.reconciliationDevices}
