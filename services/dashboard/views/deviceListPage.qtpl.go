// Code generated by qtc from "deviceListPage.qtpl". DO NOT EDIT.
// See https://github.com/valyala/quicktemplate for details.

//line views/deviceListPage.qtpl:1
package views

//line views/deviceListPage.qtpl:1
import "sensorbucket.nl/sensorbucket/pkg/api"

//line views/deviceListPage.qtpl:3
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line views/deviceListPage.qtpl:3
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line views/deviceListPage.qtpl:3
func (p *DeviceListPage) StreamBody(qw422016 *qt422016.Writer) {
//line views/deviceListPage.qtpl:3
	qw422016.N().S(`
    <div class="mx-auto w-full xl:w-2/3 flex flex-col gap-6">
        <div class="grid lg:grid-cols-4 gap-x-4 gap-y-1">
            `)
//line views/deviceListPage.qtpl:6
	StreamRenderFilters(qw422016, p.SensorGroup, false)
//line views/deviceListPage.qtpl:6
	qw422016.N().S(`
        </div>

        <div class="bg-white border rounded-md" >
            `)
//line views/deviceListPage.qtpl:10
	StreamRenderMap(qw422016, p.SensorGroup)
//line views/deviceListPage.qtpl:10
	qw422016.N().S(`
        </div>

        <div class="bg-white border rounded-md min-h-96 max-h-[90vh] lg:max-h-[50vh] overflow-y-auto">
            `)
//line views/deviceListPage.qtpl:14
	StreamRenderDeviceTable(qw422016, p.Devices, p.DevicesNextPage)
//line views/deviceListPage.qtpl:14
	qw422016.N().S(`
        </div>
    </div>
`)
//line views/deviceListPage.qtpl:17
}

//line views/deviceListPage.qtpl:17
func (p *DeviceListPage) WriteBody(qq422016 qtio422016.Writer) {
//line views/deviceListPage.qtpl:17
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/deviceListPage.qtpl:17
	p.StreamBody(qw422016)
//line views/deviceListPage.qtpl:17
	qt422016.ReleaseWriter(qw422016)
//line views/deviceListPage.qtpl:17
}

//line views/deviceListPage.qtpl:17
func (p *DeviceListPage) Body() string {
//line views/deviceListPage.qtpl:17
	qb422016 := qt422016.AcquireByteBuffer()
//line views/deviceListPage.qtpl:17
	p.WriteBody(qb422016)
//line views/deviceListPage.qtpl:17
	qs422016 := string(qb422016.B)
//line views/deviceListPage.qtpl:17
	qt422016.ReleaseByteBuffer(qb422016)
//line views/deviceListPage.qtpl:17
	return qs422016
//line views/deviceListPage.qtpl:17
}

//line views/deviceListPage.qtpl:19
func StreamRenderFilters(qw422016 *qt422016.Writer, sensorGroup *api.SensorGroup, oob bool) {
//line views/deviceListPage.qtpl:19
	qw422016.N().S(`
    <fieldset
        id="filters"
        class="relative col-span-2"
        `)
//line views/deviceListPage.qtpl:23
	if oob {
//line views/deviceListPage.qtpl:23
		qw422016.N().S(`
            hx-swap-oob="true"
        `)
//line views/deviceListPage.qtpl:25
	}
//line views/deviceListPage.qtpl:25
	qw422016.N().S(`
    >
        <label for="sensor-group-search" class="ml-1 -mb-1 block"><small class="text-xs text-slate-500">Sensor group</small></label>
        `)
//line views/deviceListPage.qtpl:28
	if sensorGroup == nil {
//line views/deviceListPage.qtpl:28
		qw422016.N().S(`
            <input
                type="text" name="search" id="sensor-group-search"

                class="block w-full px-2 py-1 border rounded-md bg-white placeholder:text-slate-600"
                hx-trigger="keyup changed delay:500ms, search"
                hx-get="`)
//line views/deviceListPage.qtpl:34
		qw422016.E().S(U("/overview/sensor-groups"))
//line views/deviceListPage.qtpl:34
		qw422016.N().S(`"
                hx-target="next ul"
                _="on blur wait 50ms add .hidden to next <ul/> on focus remove .hidden from next <ul/>"
            />
            <ul class="absolute top-full left-0 right-0 block rounded-md rounded-t-none bg-white z-[4000]
                        text-sm">
                
            </ul>
        `)
//line views/deviceListPage.qtpl:42
	} else {
//line views/deviceListPage.qtpl:42
		qw422016.N().S(`
            <a
                href="`)
//line views/deviceListPage.qtpl:44
		qw422016.E().S(U("/overview"))
//line views/deviceListPage.qtpl:44
		qw422016.N().S(`"
                hx-delete="`)
//line views/deviceListPage.qtpl:45
		qw422016.E().S(U("/overview/sensor-groups"))
//line views/deviceListPage.qtpl:45
		qw422016.N().S(`"
                hx-target="#device-table"
                class="w-full px-2 py-1 border rounded-md text-primary-600 flex justify-between items-center"
            >
                `)
//line views/deviceListPage.qtpl:49
		qw422016.E().S(sensorGroup.Name)
//line views/deviceListPage.qtpl:49
		qw422016.N().S(`
                <iconify-icon icon="charm:cross" class="text-rose-500"></iconify-icon>
            </a>
        `)
//line views/deviceListPage.qtpl:52
	}
//line views/deviceListPage.qtpl:52
	qw422016.N().S(`
    </fieldset>
`)
//line views/deviceListPage.qtpl:54
}

//line views/deviceListPage.qtpl:54
func WriteRenderFilters(qq422016 qtio422016.Writer, sensorGroup *api.SensorGroup, oob bool) {
//line views/deviceListPage.qtpl:54
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/deviceListPage.qtpl:54
	StreamRenderFilters(qw422016, sensorGroup, oob)
//line views/deviceListPage.qtpl:54
	qt422016.ReleaseWriter(qw422016)
//line views/deviceListPage.qtpl:54
}

//line views/deviceListPage.qtpl:54
func RenderFilters(sensorGroup *api.SensorGroup, oob bool) string {
//line views/deviceListPage.qtpl:54
	qb422016 := qt422016.AcquireByteBuffer()
//line views/deviceListPage.qtpl:54
	WriteRenderFilters(qb422016, sensorGroup, oob)
//line views/deviceListPage.qtpl:54
	qs422016 := string(qb422016.B)
//line views/deviceListPage.qtpl:54
	qt422016.ReleaseByteBuffer(qb422016)
//line views/deviceListPage.qtpl:54
	return qs422016
//line views/deviceListPage.qtpl:54
}

//line views/deviceListPage.qtpl:56
func StreamSensorGroupSearch(qw422016 *qt422016.Writer, sgs []api.SensorGroup) {
//line views/deviceListPage.qtpl:56
	qw422016.N().S(`
    `)
//line views/deviceListPage.qtpl:57
	for _, sg := range sgs {
//line views/deviceListPage.qtpl:57
		qw422016.N().S(`
        <li>
            <a
                href="`)
//line views/deviceListPage.qtpl:60
		qw422016.E().S(U("?sensor_group=%d", sg.Id))
//line views/deviceListPage.qtpl:60
		qw422016.N().S(`"
                hx-post="`)
//line views/deviceListPage.qtpl:61
		qw422016.E().S(U("/overview/sensor-groups?sensor_group=%d", sg.Id))
//line views/deviceListPage.qtpl:61
		qw422016.N().S(`"
                hx-target="#device-table"
                class="block hover:bg-primary-100 p-2"
            >`)
//line views/deviceListPage.qtpl:64
		qw422016.E().S(sg.Name)
//line views/deviceListPage.qtpl:64
		qw422016.N().S(`</a>
        </li>
    `)
//line views/deviceListPage.qtpl:66
	}
//line views/deviceListPage.qtpl:66
	qw422016.N().S(`
`)
//line views/deviceListPage.qtpl:67
}

//line views/deviceListPage.qtpl:67
func WriteSensorGroupSearch(qq422016 qtio422016.Writer, sgs []api.SensorGroup) {
//line views/deviceListPage.qtpl:67
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/deviceListPage.qtpl:67
	StreamSensorGroupSearch(qw422016, sgs)
//line views/deviceListPage.qtpl:67
	qt422016.ReleaseWriter(qw422016)
//line views/deviceListPage.qtpl:67
}

//line views/deviceListPage.qtpl:67
func SensorGroupSearch(sgs []api.SensorGroup) string {
//line views/deviceListPage.qtpl:67
	qb422016 := qt422016.AcquireByteBuffer()
//line views/deviceListPage.qtpl:67
	WriteSensorGroupSearch(qb422016, sgs)
//line views/deviceListPage.qtpl:67
	qs422016 := string(qb422016.B)
//line views/deviceListPage.qtpl:67
	qt422016.ReleaseByteBuffer(qb422016)
//line views/deviceListPage.qtpl:67
	return qs422016
//line views/deviceListPage.qtpl:67
}

//line views/deviceListPage.qtpl:69
func StreamRenderDeviceTable(qw422016 *qt422016.Writer, devices []api.Device, nextPage string) {
//line views/deviceListPage.qtpl:69
	qw422016.N().S(`
    <table class="w-full text-sm border-separate border-spacing-0" id="device-table">
        <thead class="text-left text-slate-500 sticky top-0 bg-white">
            <tr class="h-10">
                <th class="font-normal border-b align-middle px-4">
                    Device ID
                </th>
                <th class="font-normal border-b align-middle px-4">
                    Device Code
                </th>
                <th class="font-normal border-b align-middle px-4">
                    Device Description
                </th>
                <th class="font-normal border-b align-middle px-4">
                    Device Location Description
                </th>
            </tr>
        </thead>
        <tbody>
            `)
//line views/deviceListPage.qtpl:88
	StreamRenderDeviceTableRows(qw422016, devices, nextPage)
//line views/deviceListPage.qtpl:88
	qw422016.N().S(`
        </tbody>
    </table>
`)
//line views/deviceListPage.qtpl:91
}

//line views/deviceListPage.qtpl:91
func WriteRenderDeviceTable(qq422016 qtio422016.Writer, devices []api.Device, nextPage string) {
//line views/deviceListPage.qtpl:91
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/deviceListPage.qtpl:91
	StreamRenderDeviceTable(qw422016, devices, nextPage)
//line views/deviceListPage.qtpl:91
	qt422016.ReleaseWriter(qw422016)
//line views/deviceListPage.qtpl:91
}

//line views/deviceListPage.qtpl:91
func RenderDeviceTable(devices []api.Device, nextPage string) string {
//line views/deviceListPage.qtpl:91
	qb422016 := qt422016.AcquireByteBuffer()
//line views/deviceListPage.qtpl:91
	WriteRenderDeviceTable(qb422016, devices, nextPage)
//line views/deviceListPage.qtpl:91
	qs422016 := string(qb422016.B)
//line views/deviceListPage.qtpl:91
	qt422016.ReleaseByteBuffer(qb422016)
//line views/deviceListPage.qtpl:91
	return qs422016
//line views/deviceListPage.qtpl:91
}

//line views/deviceListPage.qtpl:94
func StreamRenderDeviceTableRows(qw422016 *qt422016.Writer, devices []api.Device, nextPage string) {
//line views/deviceListPage.qtpl:94
	qw422016.N().S(`
    `)
//line views/deviceListPage.qtpl:95
	for ix, dev := range devices {
//line views/deviceListPage.qtpl:95
		qw422016.N().S(`
    <tr
        class="hover:bg-slate-50 group"
        `)
//line views/deviceListPage.qtpl:98
		if nextPage != "" && ix == len(devices)-1 {
//line views/deviceListPage.qtpl:98
			qw422016.N().S(`
        hx-trigger="revealed"
        hx-target="this"
        hx-swap="afterend"
        hx-get="`)
//line views/deviceListPage.qtpl:102
			qw422016.E().S(nextPage)
//line views/deviceListPage.qtpl:102
			qw422016.N().S(`"
        `)
//line views/deviceListPage.qtpl:103
		}
//line views/deviceListPage.qtpl:103
		qw422016.N().S(`
    >
        <td class="px-4 h-10 border-b">`)
//line views/deviceListPage.qtpl:105
		qw422016.N().DL(dev.Id)
//line views/deviceListPage.qtpl:105
		qw422016.N().S(`</td>
        <td class="border-b"><a
            class="flex items-center px-4 h-10 text-primary-700 group-hover:underline"
            href="`)
//line views/deviceListPage.qtpl:108
		qw422016.E().S(U("/overview/devices/%d", dev.Id))
//line views/deviceListPage.qtpl:108
		qw422016.N().S(`"
            hx-target="main"
        >`)
//line views/deviceListPage.qtpl:110
		qw422016.E().S(dev.Code)
//line views/deviceListPage.qtpl:110
		qw422016.N().S(`</a></td>
        <td class="px-4 h-10 border-b">`)
//line views/deviceListPage.qtpl:111
		qw422016.E().S(dev.Description)
//line views/deviceListPage.qtpl:111
		qw422016.N().S(`</td>
        <td class="px-4 h-10 border-b">`)
//line views/deviceListPage.qtpl:112
		qw422016.E().S(dev.GetLocationDescription())
//line views/deviceListPage.qtpl:112
		qw422016.N().S(`</td>
    </tr>
    `)
//line views/deviceListPage.qtpl:114
	}
//line views/deviceListPage.qtpl:114
	qw422016.N().S(`
`)
//line views/deviceListPage.qtpl:115
}

//line views/deviceListPage.qtpl:115
func WriteRenderDeviceTableRows(qq422016 qtio422016.Writer, devices []api.Device, nextPage string) {
//line views/deviceListPage.qtpl:115
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/deviceListPage.qtpl:115
	StreamRenderDeviceTableRows(qw422016, devices, nextPage)
//line views/deviceListPage.qtpl:115
	qt422016.ReleaseWriter(qw422016)
//line views/deviceListPage.qtpl:115
}

//line views/deviceListPage.qtpl:115
func RenderDeviceTableRows(devices []api.Device, nextPage string) string {
//line views/deviceListPage.qtpl:115
	qb422016 := qt422016.AcquireByteBuffer()
//line views/deviceListPage.qtpl:115
	WriteRenderDeviceTableRows(qb422016, devices, nextPage)
//line views/deviceListPage.qtpl:115
	qs422016 := string(qb422016.B)
//line views/deviceListPage.qtpl:115
	qt422016.ReleaseByteBuffer(qb422016)
//line views/deviceListPage.qtpl:115
	return qs422016
//line views/deviceListPage.qtpl:115
}

//line views/deviceListPage.qtpl:117
func StreamRenderMap(qw422016 *qt422016.Writer, sensorGroup *api.SensorGroup) {
//line views/deviceListPage.qtpl:117
	qw422016.N().S(`
    <div 
        hx-ext="leaflet"
        class="w-full h-96"
        id="device-map"
    >
        <script type="text/javascript">
        function getWebSocketURL(path) {
            const loc = window.location;
            let newUri;

            if (loc.protocol === "https:") {
                newUri = "wss:";
            } else {
                newUri = "ws:";
            }
            
            newUri += "//" + loc.host + path;
            
            return newUri;
        }
        (() => {
            const devmap = htmx.find("#device-map")
            let map;
            let markerLayer;
            let ws;

            function replaceDevices() {
                // Remove old
                if (ws != undefined) ws.close();
                if (markerLayer != undefined) markerLayer.remove()

                // New layer
                markerLayer = L.markerClusterGroup()
                map.addLayer(markerLayer);

                // Fetch news
                ws = new WebSocket(getWebSocketURL("`)
//line views/deviceListPage.qtpl:154
	qw422016.E().S(U("/overview/devices/stream-map"))
//line views/deviceListPage.qtpl:154
	qw422016.N().S(`" + location.search))
                ws.onmessage = (event) => {
                    const data = JSON.parse(event.data)
                    const marker = L.marker(data.coordinates).addTo(markerLayer)
                    marker.on('click', evt => {
                        const url = "`)
//line views/deviceListPage.qtpl:159
	qw422016.E().S(U("/overview/devices/"))
//line views/deviceListPage.qtpl:159
	qw422016.N().S(`" + data.device_id;
                        htmx.ajax("GET", url, "main").then(() => {
                            window.history.pushState({}, null, url)
                        })
                    })
                    marker.bindTooltip(data.device_code)
                }
            }
            htmx.on(document.body, "newDeviceList", () => {
                replaceDevices()
            })
            async function init() {
                map = devmap.leaflet;
                replaceDevices()
            }

            if (devmap.leaflet !== undefined) {
                init()
            } else {
                devmap.addEventListener('leaflet:init', () => init())
            }
        })()
    </script>
    </div>
`)
//line views/deviceListPage.qtpl:183
}

//line views/deviceListPage.qtpl:183
func WriteRenderMap(qq422016 qtio422016.Writer, sensorGroup *api.SensorGroup) {
//line views/deviceListPage.qtpl:183
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/deviceListPage.qtpl:183
	StreamRenderMap(qw422016, sensorGroup)
//line views/deviceListPage.qtpl:183
	qt422016.ReleaseWriter(qw422016)
//line views/deviceListPage.qtpl:183
}

//line views/deviceListPage.qtpl:183
func RenderMap(sensorGroup *api.SensorGroup) string {
//line views/deviceListPage.qtpl:183
	qb422016 := qt422016.AcquireByteBuffer()
//line views/deviceListPage.qtpl:183
	WriteRenderMap(qb422016, sensorGroup)
//line views/deviceListPage.qtpl:183
	qs422016 := string(qb422016.B)
//line views/deviceListPage.qtpl:183
	qt422016.ReleaseByteBuffer(qb422016)
//line views/deviceListPage.qtpl:183
	return qs422016
//line views/deviceListPage.qtpl:183
}

//line views/deviceListPage.qtpl:186
type DeviceListPage struct {
	BasePage
	Devices         []api.Device
	DevicesNextPage string
	SensorGroup     *api.SensorGroup
}
