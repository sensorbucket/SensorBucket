...
func (ts *PSQLTenantStore) GetTenantByID(ctx context.Context, id int64) (*tenants.Tenant, error) {
...
func (ts *PSQLTenantStore) Create(ctx context.Context, tenant *tenants.Tenant) error {
	q := sq.Insert("tenants").
		Columns(
			"name",
			"address",
...
func (ts *PSQLTenantStore) Update(ctx context.Context, tenant *tenants.Tenant) error {
...
func (store *PSQLTenantStore) GetTenantHierarchyChildren(
	...
	return getTenantHierarchy(ctx, store.db, anchor)
}

func (store *PSQLTenantStore) GetUserTenants(
	ctx context.Context,
	userID string,
) ([]tenants.Tenant, error) {
	anchor := sq.Select("t.*").
...
func getTenantHierarchy(
	ctx context.Context,
	db *sqlx.DB,
	anchor sq.SelectBuilder,
) ([]tenants.Tenant, error) {
	recursive := sq.Select("t.*").
		From("tenants t").
		InnerJoin("children c ON t.parent_tenant_id = c.id")
...
func (store *PSQLTenantStore) GetMember(
	ctx context.Context,
	tenantID int64,
	userID string,
) (*tenants.Member, error) {
...
func (store *PSQLTenantStore) IsMember(
	ctx context.Context,
	tenantID int64,
	userID string,
	explicit bool,
...
func (s *PSQLTenantStore) SaveMember(
	ctx context.Context,
	tenantID int64,
	...
	if count > 0 {
		return s.updateMember(ctx, tenantID, member)
	}
	return s.createMember(ctx, tenantID, member)
}

func (s *PSQLTenantStore) createMember(
	ctx context.Context,
	tenantID int64,
	member *tenants.Member,
) error {
...
func (s *PSQLTenantStore) updateMember(
	ctx context.Context,
	tenantID int64,
	member *tenants.Member,
) error {
...
func (s *PSQLTenantStore) RemoveMember(ctx context.Context, tenantID int64, userID string) error {
