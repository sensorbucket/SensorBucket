...
func TestNewApiKeyNoName(t *testing.T) {
	...
	req, _ := http.NewRequest(
		"POST",
		"/api-keys",
		strings.NewReader(`{"name": "", "tenant_id": 905}`),
	)
...
func TestNewApiKeyNoOrganisationID(t *testing.T) {
	...
	req, _ := http.NewRequest(
		"POST",
		"/api-keys",
		strings.NewReader(`{"name": "wasdasdas", "tenant_id": 0}`),
	)
	req.Header.Add("content-type", "application/json")
...
func TestNewApiKeyPermissionsNotGiven(t *testing.T) {
	...
	req, _ := http.NewRequest(
		...
		strings.NewReader(
			fmt.Sprintf(
				`{"name": "wasdasdas", "tenant_id": 12, "expiration_date": "%s"}`,
...
func TestNewApiKeyPermissionsEmptyList(t *testing.T) {
	...
	req, _ := http.NewRequest(
		...
		strings.NewReader(
			fmt.Sprintf(
				`{"name": "wasdasdas", "tenant_id": 12, "permissions":[], "expiration_date": "%s"}`,
				time.Now().Add(time.Hour*24).Format(time.RFC3339),
			),
		),
	)
...
func TestNewApiKeyExpirationDateNotInTheFuture(t *testing.T) {
	...
	req, _ := http.NewRequest(
		...
		strings.NewReader(
			fmt.Sprintf(
				`{"name": "wasdasdas", "tenant_id": 12, "permissions":["WRITE_DEVICES"], "expiration_date": "%s"}`,
				time.Now().Add(-time.Hour*24).Format(time.RFC3339),
			),
		),
	)
...
func TestNewApiKeyIsCreatedWithExpirationDate(t *testing.T) {
	...
	req, _ := http.NewRequest(
		...
		strings.NewReader(
			fmt.Sprintf(
				`{"name": "whatever", "permissions":["WRITE_DEVICES"], "tenant_id": 905, "expiration_date": "%s"}`,
				exp.Format("2006-01-02T15:04:05.999999999Z"),
			),
		),
	)
	req.Header.Add("content-type", "application/json")
...
func TestAuthenticateApiKeyWithPermissions(t *testing.T) {
	...
	assert.Equal(
		t,
		`{"extra":{"perms":["READ_DEVICES","WRITE_DEVICES","READ_API_KEYS","WRITE_API_KEYS"],"tid":431}}`+"\n",
		rr.Body.String(),
	)
	assert.Len(t, svc.AuthenticateApiKeyCalls(), 1)
}
...
func TestAuthenticateApiKeyIsValidWithExpirationDate(t *testing.T) {
	...
	assert.Equal(
		t,
		fmt.Sprintf(`{"extra":{"exp":%d,"perms":[],"tid":431}}`+"\n", exp),
		rr.Body.String(),
	)
	assert.Len(t, svc.AuthenticateApiKeyCalls(), 1)
}
