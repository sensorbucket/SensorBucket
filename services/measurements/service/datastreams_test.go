...
func TestFindOrCreateDatastreamWorks(t *testing.T) {
	store := &DatastreamFinderCreaterMock{
		FindDatastreamFunc: func(sensorID int64, observationProperty string) (*service.Datastream, error) {
	...
	ds, err := service.FindOrCreateDatastream(sensorID, obs, uom, store)
